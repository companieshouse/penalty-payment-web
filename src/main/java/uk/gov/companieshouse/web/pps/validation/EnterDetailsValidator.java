...
public class EnterDetailsValidator {
    ...
    private static final String ROE_PENALTY_REF_REGEX = "^[Uu]\\d{7}$";
    ...
    public void isValidCompanyNumber(final EnterDetails enterDetails, final BindingResult bindingResult) {
        ...
        if (enterDetails.getCompanyNumber() == null || enterDetails.getCompanyNumber().isEmpty()) {
            ...
            bindingResult.rejectValue(companyNumberString, companyNumberString, bundle.getString(key));
            ...
            String regex = ROE.name().equals(penaltyReferenceName) ? OVERSEAS_ENTITY_ID_REGEX : COMPANY_NUMBER_REGEX;
    ...
    public void isValidPenaltyRef(final EnterDetails enterDetails, final BindingResult bindingResult) {
        ...
        if (StringUtils.isBlank(penaltyRef)) {
            bindingResult.rejectValue(penaltyRefString, penaltyRefString, bundle.getString("enterDetails.penaltyRef.notEmpty"));
        } else if (StringUtils.contains(penaltyRef, " ")) {
            bindingResult.rejectValue(penaltyRefString, penaltyRefString, bundle.getString("enterDetails.penaltyRef.noSpaces"));
        } else {
            String regex = switch (PenaltyReference.valueOf(penaltyReferenceName)) {
                ...
                case ROE -> ROE_PENALTY_REF_REGEX;
            };
            if (!penaltyRef.matches(regex)) {
                bindingResult.rejectValue(penaltyRefString, penaltyRefString, bundle.getString("enterDetails.penaltyRef.notValid"));
            }
        }
    }
}
